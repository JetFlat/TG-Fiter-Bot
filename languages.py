...
async def handle_forwarded_and_direct(message: types.Message, state: FSMContext):
    ...
    # Определение типа контента и сохранение file_id
    if message.photo:
        content_type = 'photo'
    ...
    if message.forward_from_chat or message.forward_from or message.forward_sender_name:
        ...
        if not note_content:
            note_content = "Пересланное сообщение"

        # Сохраняем информацию о типе контента для пересланного сообщения
        content_type = f"forwarded_{content_type}" if content_type else "forwarded"

        # Логирование для отладки
        logging.info(
            f"Сохраняем пересланное сообщение: тип={content_type}, file_id={file_id}, forward_chat_id={forward_chat_id}")
...
async def display_note(chat_id, note, index, total, state: FSMContext = None):
    ...
    # Логирование для отладки
    logging.info(f"Отображаем заметку: тип={content_type}, file_id={file_id}, forward_chat_id={forward_chat_id}")
    ...
    try:
        ...
        if content_type.startswith('forwarded') and forward_chat_id and forward_message_id:
            try:
                # Пытаемся скопировать оригинальное сообщение
                await bot.copy_message(
                    chat_id=chat_id,
                    from_chat_id=forward_chat_id,
                    message_id=forward_message_id
                )
                logging.info("Сообщение успешно скопировано")
            except Exception as e:
                logging.error(f"Ошибка при копировании сообщения: {e}")

                # Извлекаем базовый тип контента (без префикса 'forwarded_')
                base_content_type = content_type.replace('forwarded_', '') if '_' in content_type else 'text'

                # Если есть file_id, отправляем файл напрямую
                if file_id:
                    if base_content_type == 'photo' or content_type == 'forwarded_photo':
                        await bot.send_photo(chat_id, file_id, caption=note_content or None)
                    elif base_content_type == 'video' or content_type == 'forwarded_video':
                        await bot.send_video(chat_id, file_id, caption=note_content or None)
                    elif base_content_type == 'document' or content_type == 'forwarded_document':
                        await bot.send_document(chat_id, file_id, caption=note_content or None)
                    elif base_content_type == 'audio' or content_type == 'forwarded_audio':
                        await bot.send_audio(chat_id, file_id, caption=note_content or None)
                    elif base_content_type == 'voice' or content_type == 'forwarded_voice':
                        await bot.send_voice(chat_id, file_id, caption=note_content or None)
                    elif base_content_type == 'sticker' or content_type == 'forwarded_sticker':
                        await bot.send_sticker(chat_id, file_id)
                    else:
                        await bot.send_message(chat_id, note_content or "Пересланное сообщение (контент недоступен)")
                else:
                    await bot.send_message(chat_id, note_content or "Пересланное сообщение (контент недоступен)")

        # Обработка обычных сообщений
        elif content_type == 'text':
            await bot.send_message(chat_id, note_content or "Пустая текстовая заметка")
            ...
            await bot.send_message(chat_id, note_content or "Заметка (контент недоступен)")

    except Exception as e:
        await bot.send_message(chat_id, f'⚠️ Ошибка отображения заметки: {str(e)[:50]}')
        logging.error(f'Ошибка отображения заметки: {e}')

    buttons = [
        [
            InlineKeyboardButton(text="🗑️ Удалить", callback_data=f"delete_note_{note['id']}")
        ],
        [
            InlineKeyboardButton(text="⬅️ Назад", callback_data=f"note_nav_{index - 1}"),
            InlineKeyboardButton(text=f"{index + 1}/{total}", callback_data="note_count"),
            InlineKeyboardButton(text="➡️ Вперёд", callback_data=f"note_nav_{index + 1}")
        ]
    ]
